            ...
            <table>
              ...
              <tbody>
                {topUnderwriters.underwriterPositions.map((uw) => {
                  const uwHf = Number(uw.healthFactor) / 10000
                  const uwHfClass = uwHf >= 1.5 ? 'health-green' : uwHf >= 0.85 ? 'health-yellow' : 'health-red'
                  return (
                    <tr key={uw.id}>
                      <td className="mono">{uw.underwriter.slice(0, 6)}...{uw.underwriter.slice(-4)}</td>
                      <td>${formatUsdc(BigInt(uw.collateral))}</td>
                      <td className={uwHfClass}>{uwHf.toFixed(2)}</td>
                    </tr>
                  )
                })}
              </tbody>
